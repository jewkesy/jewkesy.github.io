...
function buildAmazonReview(data) {
	...
	var arIds = ['pc_uk_stars', 'pc_us_stars', 'pc_de_stars', 'pc_in_stars', 'pc_ca_stars', 'pc_jp_stars', 'pc_au_stars', 'pc_fr_stars', 'pc_es_stars', 'pc_it_stars', 'pc_mx_stars', 'pc_br_stars', 'pc_es_us_stars', 'pc_ga_stars'];
	...
	for (var i = 0; i < arIds.length; i++) {
		...
		else if (i ==11) e.classList.add(getCssStar(data.br.score));
		else if (i ==12) e.classList.add(getCssStar(data.es_us.score));
		else if (i ==13) e.classList.add(getCssStar(Math.ceil(data.ga.score)));
	}
